...
public final class AdaptyRevenueService: AdaptyRevenueServiceProtocol, @unchecked Sendable {
    ...
    public func fetchAllProducts() async throws -> [any ProductProtocol] {
        ...
        if !hadSuccess, let err = lastError {
            throw mapToRevenueError(err)
        }
    ...
    public func fetchProducts(forPaywall id: String) async throws -> [any ProductProtocol] {
        do {
            ...
            let items = try await Adapty.getPaywallProducts(paywall: paywall)
    ...
    public func purchase(productID: String) async throws -> any PurchaseResultProtocol {
        ...
        do {
            ...
            let mapped = mapToRevenueError(error)
            emit(AdaptyServiceEvent(kind: .purchaseFailed(productID: productID, error: "\(mapped)")))
            throw mapped
        }
    }
    ...
    public func setCustomerUserId(_ id: String) async throws {
        do { try await Adapty.identify(id) }
        catch { throw mapToRevenueError(error) }
    }

    public func logout() async throws {
        do { try await Adapty.logout() }
        catch { throw mapToRevenueError(error) }
    }


    public func updateAttribution(_ data: [AnyHashable: Any], from source: String) async throws {
        do { try await Adapty.updateAttribution(data, source: source) }
        catch { throw mapToRevenueError(error) }
    }
    ...
    private func resolvePaywallProduct(for productID: String) async throws -> AdaptyPaywallProduct {
        ...
        if let err = lastError { throw mapToRevenueError(err) }
...
extension AdaptyRevenueService {
    private func mapToRevenueError(_ error: Error) -> RevenueError {
        if let adapty = error as? AdaptyError {
            return mapToRevenueError(adapty)
        }
        if let sk = error as? SKError {
            ...
            default:
                return .unknown(underlying: sk.localizedDescription)
            }
        }
    ...
    private func mapToRevenueError(_ adapty: AdaptyError) -> RevenueError {
        switch adapty.adaptyErrorCode {
        case .unknown:
            return .unknown(underlying: adapty.description)
